...
class MinMaxQLearningAgent(SARSAgent):
    def choose_action(self, state):
        if random.uniform(0, 1) < self.epsilon:
            return random.choice(self.actions)
        else:
    ...
    def learn(self, state, action, reward, next_state, _=None):
        # For compatibility with SARSA's learn method signature
        next_min_action = np.argmin(self.q_table[next_state])
        super().learn(state, action, reward, next_state, next_min_action)

def simulate_episodes(env, num_episodes=1000):
    husband_agent = SARSAgent(env.action_space)
    ...
    for episode in range(num_episodes):
        ...
        next_state = state  # Assuming the state does not change
        next_action_husband = husband_agent.choose_action(next_state)
        next_action_wife = wife_agent.choose_action(next_state)

        husband_agent.learn(state, action_husband, husband_reward, next_state, next_action_husband)
...
def plot_cumulative_rewards(husband_cumulative_rewards, wife_cumulative_rewards):
    plt.plot(husband_cumulative_rewards, label='Husband (SARSA)')
